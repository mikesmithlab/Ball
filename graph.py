...
if __name__ == '__main__':
    ...
    for path in pathnames:
        ...
        Headings = ['Video','Grit','Roughness','Voltage','Acceleration','ballHeightMM','-','+']
        write_row_to_csv(filenames[1][:-5]+'height_histogram.csv',Headings,append=False)
        print(filenames)
        for filename in filenames:
            ...
            #a=calcBasics(data,'xVelMM')
            #b=calcBasics(data,'yVelMM')
            c=calcBasics(data,'ballHeightMM')
            #d=calcBasics(data,'omega_k')
            #e=calcBasics(data, 'omega_i')
            if '80' in filename:
                if '077' in filename:
                    values =  [filename,'80','201','077','3.25'] + c
                if '062' in filename:
                    values =  [filename,'80','201','0.62','2.75'] + c
                if '050' in filename:
                    values =  [filename,'80','201','0.5','2.2'] + c
                if '040' in filename:
                    values =  [filename,'80','201','0.4','1.9'] + c
            if '120' in filename:
                if '077' in filename:
                    values =  [filename,'120','124','077','3.25'] + c
                if '062' in filename:
                    values =  [filename,'120','124','0.62','2.75'] + c
                if '050' in filename:
                    values =  [filename,'120','124','0.5','2.2'] + c
                if '040' in filename:
                    values =  [filename,'120','124','0.4','1.9'] + c
            if '240' in filename:
                if '077' in filename:
                    values =  [filename,'240','58.5','077','3.25'] + c
                if '062' in filename:
                    values =  [filename,'240','58.5','0.62','2.75'] + c
                if '050' in filename:
                    values =  [filename,'240','58.5','0.5','2.2'] + c
                if '040' in filename:
                    values =  [filename,'240','58.5','0.4','1.9'] + c
            
            if '400' in filename:
                if '077' in filename:
                    values =  [filename,'400','35','077','3.25'] + c
                if '062' in filename:
                    values =  [filename,'400','35','0.62','2.75'] + c
                if '050' in filename:
                    values =  [filename,'400','35','0.5','2.2'] + c
                if '040' in filename:
                    values =  [filename,'400','35','0.4','1.9'] + c
            
            if '800' in filename:
                if '077' in filename:
                    values =  [filename,'800','21.6','077','3.25'] + c
                if '062' in filename:
                    values =  [filename,'800','21.6','0.62','2.75'] + c
                if '050' in filename:
                    values =  [filename,'800','21.6','0.5','2.2'] + c
                if '040' in filename:
                    values =  [filename,'800','21.6','0.4','1.9'] + c
            
            # 0.77 =  3.25g, 0.62 = 2.75g, 0.5 = 2.2g, 0.4 = 1.9g
